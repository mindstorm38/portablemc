 ...
 */
const char          *pmc_err_message(const pmc_err *err);

/*
 * Free the given error structure if not NULL.
 */
void                 pmc_err_free(pmc_err *err);

/*
 * Create a new authenticator with the given application (client) id.
 */
pmc_msa_auth        *pmc_msa_auth_new(const char *app_id);
const char          *pmc_msa_auth_app_id(const pmc_msa_auth *auth);
const char          *pmc_msa_auth_language_code(const pmc_msa_auth *auth);
...
void                 pmc_msa_auth_free(pmc_msa_auth *auth);

const char          *pmc_msa_device_code_flow_app_id(const pmc_msa_device_code_flow *flow);
const char          *pmc_msa_device_code_flow_user_code(const pmc_msa_device_code_flow *flow);
const char          *pmc_msa_device_code_flow_verification_uri(const pmc_msa_device_code_flow *flow);
const char          *pmc_msa_device_code_flow_message(const pmc_msa_device_code_flow *flow);
pmc_msa_account     *pmc_msa_device_code_flow_wait(const pmc_msa_device_code_flow *flow, pmc_err **err);
void                 pmc_msa_device_code_flow_free(pmc_msa_device_code_flow *flow);

const char          *pmc_msa_account_app_id(const pmc_msa_account *acc);
const char          *pmc_msa_account_access_token(const pmc_msa_account *acc);
const pmc_uuid      *pmc_msa_account_uuid(const pmc_msa_account *acc);
const char          *pmc_msa_account_username(const pmc_msa_account *acc);
const char          *pmc_msa_account_xuid(const pmc_msa_account *acc);
...
void                 pmc_msa_account_free(pmc_msa_account *acc);

pmc_msa_database    *pmc_msa_database_new(const char *path);
const char          *pmc_msa_database_file(const pmc_msa_database *database);
...
void                 pmc_msa_database_free(pmc_msa_database *database);
